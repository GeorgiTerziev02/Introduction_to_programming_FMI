...
void initBoardWithValue(int board[ROWS][COLS], int rows, int cols, int value) {
...
bool isPopulatedField(int x, int y, int board[ROWS][COLS]) {
	return board[x][y] != INITIAL_VALUE;
}

void enterValidCoordinates(int& x, int& y, int board[ROWS][COLS]) {
	do {
		std::cin >> x >> y;
	} while (!isInsideBoard(x, y) || isPopulatedField(x, y, board));
}

void printMatrix(int board[ROWS][COLS], int rows, int cols) {
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			std::cout << board[i][j];
		}
...
bool hasWinnerOnRows(int board[ROWS][COLS], int rows, int cols) {
	for (int i = 0; i < rows; i++) {
		bool isFullRow = true;
...
bool hasWinnerOnCols(int board[ROWS][COLS], int rows, int cols) {
	for (int j = 0; j < COLS; j++) {
		bool isFullCol = true;
...
bool hasWinnerOnMainDiagonal(int board[ROWS][COLS], int rows, int cols) {
	for (int i = 0; i < rows - 1; i++) {
...
bool hasWinnerOnSecondaryDiagonal(int board[ROWS][COLS], int rows, int cols) {
...
bool hasWinner(int board[ROWS][COLS], int rows, int cols) {
